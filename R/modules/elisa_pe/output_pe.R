...
#' Create standardized ELISA-PE output
...
#'
#' @param interpretation_result Result from interpret_elisa_pe()
#' @return Tibble in standardized format
#' @export
output_elisa_pe <- function(interpretation_result) {
  cat(sprintf("📤 OUTPUT (PE): Creating standardized format\n"))
...
#' Process ELISA-PE file end-to-end
...
#' @export
process_elisa_pe_file <- function(file_path,
                                    qc_settings = elisa_default_qc_settings()) {
  cat(sprintf("\n═══════════════════════════════════════════════\n"))
  cat(sprintf("Processing ELISA-PE: %s\n", basename(file_path)))
  cat(sprintf("═══════════════════════════════════════════════\n"))

  # STEP 1: Ingestion
  ingestion_result <- ingest_elisa_pe(file_path)

  # STEP 2: QC
  qc_result <- qc_elisa_pe(ingestion_result, qc_settings)

  # STEP 3: Interpretation
  interpretation_result <- interpret_elisa_pe(qc_result)

  # STEP 4: Output
  results_clean <- output_elisa_pe(interpretation_result)

  cat(sprintf("═══════════════════════════════════════════════\n"))
  cat(sprintf("✅ ELISA-PE processing complete\n"))
  cat(sprintf("═══════════════════════════════════════════════\n\n"))
