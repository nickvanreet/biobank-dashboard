...
#' Main ELISA-VSG QC function
#' @param ingestion_result Result from ingest_elisa_vsg()
...
#' @export
qc_elisa_vsg <- function(ingestion_result, qc_settings = elisa_default_qc_settings()) {
  cat(sprintf("🔍 QC VALIDATION (VSG): %s\n", ingestion_result$metadata$plate_id))
