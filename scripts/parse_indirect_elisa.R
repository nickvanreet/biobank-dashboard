...
read_elisa_layout <- function(path) {
  ...
  # ----- Samples -----
  df_samples <- tryCatch(
    read_excel(path, sheet = "Results") %>% clean_names(),
    error = function(e) {
      stop(sprintf("Failed to read 'Results' sheet from '%s': %s",
                   basename(path), conditionMessage(e)))
    }
  )

  if (nrow(df_samples) == 0) {
    warning(sprintf("No sample data found in 'Results' sheet of '%s'", basename(path)))
  }
  ...
  df_controls <- tryCatch(
    read_excel(path, sheet = "Controls") %>% clean_names(),
    error = function(e) {
      stop(sprintf("Failed to read 'Controls' sheet from '%s': %s",
                   basename(path), conditionMessage(e)))
    }
  )

  if (nrow(df_controls) == 0) {
    warning(sprintf("No control data found in 'Controls' sheet of '%s'", basename(path)))
  }
